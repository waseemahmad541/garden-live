    ...
    <main className="min-h-screen overflow-hidden bg-[#f3f7f0] text-botanical-black">
      ...
      <div className="min-h-screen transition-colors duration-500 peer-checked/theme:bg-[#07130d] peer-checked/theme:text-white">
        <Navbar
          ...
          className="border-white/30 bg-white/68 shadow-[0_10px_45px_rgba(10,40,22,0.08)] backdrop-blur-2xl peer-checked/theme:border-white/10 peer-checked/theme:bg-[#07130d]/70"
        />
        <ThemeSwitch />
        {children}
        <FloatingActions />
  ...
  return (
    <>
      <Hero
        ...
        description="A premium digital garden membership platform for luxury landscaping, AI Plant Doctor, QR Plant Passport, Smart Garden IoT, plant nursery, garden maintenance and beautifully accountable green living."
        image={img.home}
  ...
  return (
    <section className="group/hero relative isolate min-h-[100svh] overflow-hidden bg-[#06120b] text-white">
      <div className="absolute inset-0 transition duration-[1600ms] group-hover/hero:scale-[1.025]">
        <Image src={image} alt={`${eyebrow} by Garden Live`} fill priority sizes="100vw" className="object-cover opacity-68 saturate-[1.08] contrast-[1.04]" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_78%_18%,rgba(183,230,110,0.34),transparent_28%),radial-gradient(circle_at_24%_70%,rgba(31,91,58,0.36),transparent_34%),linear-gradient(108deg,rgba(4,16,9,0.98),rgba(10,44,25,0.78)_46%,rgba(4,16,9,0.38))]" />
      ...
      <div className="gl-container relative grid min-h-[100svh] items-center gap-12 py-28 lg:grid-cols-[1.02fr_0.98fr]">
        <div className="gl-rise max-w-5xl">
          <Badge tone="premium" className="rounded-full border border-white/25 bg-white/14 px-4 py-2 text-white shadow-[0_18px_60px_rgba(0,0,0,0.22)] backdrop-blur-xl">
            {eyebrow}
          </Badge>
          <h1 className="gl-headline mt-7 text-5xl font-semibold leading-[0.92] tracking-[0] sm:text-7xl lg:text-[5.9rem]">{title}</h1>
          ...
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            ...
            <Button asChild size="lg" variant="secondary" className="h-12 rounded-full border-white/25 bg-white/12 px-7 text-white backdrop-blur-xl hover:bg-white/20" rightIcon={<ArrowRight className="h-4 w-4" aria-hidden />}>
...
function HeroStats() {
  return (
    <div className="mt-10 grid max-w-2xl gap-3 sm:grid-cols-3">
      {[["AI Health", "96/100"], ["Garden Visits", "24h visible"], ["Green Promise", "Eligible care"]].map(([label, value]) => (
        <div key={label} className="rounded-2xl border border-white/18 bg-white/10 p-4 shadow-[0_18px_60px_rgba(0,0,0,0.16)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:bg-white/15">
          <p className="text-xs uppercase tracking-[0.18em] text-white/55">{label}</p>
          <p className="gl-count mt-2 text-2xl font-semibold text-botanical-lime">{value}</p>
        </div>
      ))}
    </div>
  );
...
function HeroPhoneMockup() {
  ...
  return (
    <aside className="gl-rise gl-delay-1 relative mx-auto w-full max-w-[520px] lg:ml-auto">
      ...
      <div className="mx-auto rounded-[3rem] border border-white/20 bg-white/14 p-3 shadow-[0_45px_140px_rgba(0,0,0,0.46)] backdrop-blur-2xl transition duration-700 group-hover/hero:-translate-y-3 group-hover/hero:rotate-1">
        <div className="overflow-hidden rounded-[2.35rem] border border-white/10 bg-[#07130d]">
          <div className="relative h-[620px]">
            <Image src={img.ai} alt="AI Plant Doctor phone mockup for Garden Live" fill sizes="(min-width: 1024px) 420px, 90vw" className="object-cover opacity-80" />
            ...
            <div className="absolute inset-x-5 bottom-5 rounded-[1.8rem] border border-white/12 bg-white/12 p-5 text-white backdrop-blur-2xl">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-white/50">AI Plant Doctor</p>
                  <h2 className="mt-2 text-2xl font-semibold">Areca Palm scan</h2>
                </div>
              ...
              <div className="mt-4 h-2 overflow-hidden rounded-full bg-white/15"><div className="h-full w-[92%] rounded-full bg-botanical-lime shadow-[0_0_30px_rgba(183,230,110,0.6)]" /></div>
...
function FloatingMiniCard({ className, label, value }: { className: string; label: string; value: string }) {
  return (
    <div className={`absolute rounded-3xl border border-white/18 bg-white/12 p-4 text-white shadow-[0_30px_90px_rgba(0,0,0,0.3)] backdrop-blur-2xl ${className}`}>
      <p className="text-xs uppercase tracking-[0.18em] text-white/55">{label}</p>
      <p className="mt-2 text-3xl font-semibold text-botanical-lime">{value}</p>
    </div>
  );
}
...
function ExperienceGrid() {
  return (
    <section id="premium-services" className="gl-reveal py-24">
      <div className="gl-container">
        <SectionHeading eyebrow="Premium Services" title="A luxury product layer for every Garden Live promise." description="Landscape design, nursery supply, rooftops, villa gardens, AI diagnosis, QR passport and store workflows now feel like one premium technology platform." />
        <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {services.map(([title, href, Icon, image, description]) => <ImageCard key={href} title={title} href={href} icon={Icon} image={image} description={description} />)}
        </div>
      </div>
    </section>
  );
...
function ImageCard({ title, href, icon: Icon, image, description }: { title: string; href: string; icon: LucideIcon; image: string; description: string }) {
  return (
    <Link href={href} className="group overflow-hidden rounded-[2rem] border border-white/80 bg-white/74 shadow-[0_24px_80px_rgba(16,67,38,0.10)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:shadow-[0_34px_100px_rgba(16,67,38,0.18)]">
      <div className="relative h-64">
        <Image src={image} alt={`${title} by Garden Live`} fill sizes="(min-width: 1280px) 33vw, 100vw" className="object-cover transition duration-700 group-hover:scale-105" />
        <div className="absolute inset-0 bg-gradient-to-t from-botanical-black/60 to-transparent" />
        <Icon className="absolute bottom-5 left-5 h-7 w-7 text-white" aria-hidden />
      </div>
      <div className="p-6">
        <h2 className="text-2xl font-semibold">{title}</h2>
        <p className="mt-3 text-sm leading-7 text-neutral-slate">{description}</p>
        <span className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-botanical-green">Explore <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" aria-hidden /></span>
      </div>
    </Link>
  );
}
...
function MembershipShowcase() {
  ...
  return (
    <section className="gl-reveal bg-white/60 py-24 backdrop-blur">
      <div className="gl-container">
        <SectionHeading eyebrow="Membership Plans" title="Pricing cards that feel like a premium subscription product." description="Each plan connects visits, records, AI recommendations, plant replacement eligibility and rewards." />
        <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-5">
          {plans.map(([name, price, detail], index) => (
            <Link href="/membership-plans" key={name} className={`group rounded-[2rem] border p-6 shadow-[0_18px_70px_rgba(16,67,38,0.09)] transition hover:-translate-y-2 ${index === 3 ? "border-botanical-green bg-gradient-to-br from-botanical-mint via-white to-white" : "border-white/80 bg-white/76 backdrop-blur-xl"}`}>
              <Badge tone={index === 3 ? "success" : "info"}>{index === 3 ? "Popular" : "Garden Live"}</Badge>
              <h3 className="mt-5 text-2xl font-semibold">{name}</h3>
              <p className="mt-3 text-3xl font-semibold">{price}</p>
              <p className="mt-3 min-h-12 text-sm leading-6 text-neutral-slate">{detail}</p>
              {["AI care insights", "Digital visit record", "Green Promise review"].map((item) => <p key={item} className="mt-3 flex items-center gap-2 text-sm font-medium"><CheckCircle2 className="h-4 w-4 text-botanical-green" aria-hidden />{item}</p>)}
              <span className="mt-7 inline-flex items-center gap-2 text-sm font-semibold text-botanical-green">Compare plan <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" aria-hidden /></span>
            </Link>
          ))}
        </div>
      </div>
...
function HomepageStats() {
  ...
  return (
    <section className="gl-reveal py-24">
      <div className="gl-container">
        <div className="relative overflow-hidden rounded-[2.5rem] bg-[#07130d] p-6 text-white shadow-[0_34px_120px_rgba(7,19,13,0.28)] sm:p-8 lg:p-12">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(183,230,110,0.22),transparent_30%),radial-gradient(circle_at_85%_60%,rgba(255,255,255,0.14),transparent_28%)]" />
          <div className="relative grid gap-10 lg:grid-cols-[0.82fr_1.18fr] lg:items-end">
            <div><Badge tone="premium" className="bg-white/15 text-white">Statistics</Badge><h2 className="mt-5 text-4xl font-semibold leading-tight sm:text-6xl">Garden intelligence that looks calm, but works hard.</h2><p className="mt-5 text-base leading-8 text-white/68">Memberships, AI reports, plant passports, visits and rewards are presented with clear operational signals.</p></div>
            <div className="grid gap-4 sm:grid-cols-2">{stats.map(([value, label]) => <div key={label} className="rounded-[1.75rem] border border-white/10 bg-white/10 p-6 backdrop-blur-xl transition hover:-translate-y-1 hover:bg-white/15"><p className="gl-count text-5xl font-semibold text-botanical-lime">{value}</p><p className="mt-3 text-sm font-medium text-white/68">{label}</p></div>)}</div>
          </div>
        </div>
      </div>
    </section>
  ...
  return (
    <section className="gl-reveal py-24">
      <div className="gl-container grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <div><Badge tone="premium">QR Plant Passport</Badge><h2 className="mt-5 text-4xl font-semibold leading-tight sm:text-6xl">Every plant gets a luxury digital identity.</h2><p className="mt-5 text-base leading-8 text-neutral-slate">Plant details, warranty, Green Promise status, care timeline, growth history, AI diagnosis and replacement eligibility in one scannable profile.</p></div>
        <div className="rounded-[2rem] border border-white/80 bg-white/76 p-5 shadow-[0_30px_100px_rgba(16,67,38,0.14)] backdrop-blur-xl"><div className="grid gap-4 sm:grid-cols-[0.85fr_1.15fr]"><div className="relative min-h-80 overflow-hidden rounded-[1.5rem]"><Image src={img.passport} alt="QR Plant Passport premium plant profile" fill sizes="(min-width: 1024px) 35vw, 100vw" className="object-cover" /></div><div className="rounded-[1.5rem] bg-[#07130d] p-5 text-white"><QrCode className="h-12 w-12 text-botanical-lime" aria-hidden /><p className="mt-5 text-2xl font-semibold">Monstera Deliciosa</p><p className="mt-2 text-sm text-white/65">Passport ID GL-PLANT-0924</p>{["Warranty active", "Growth update due in 7 days", "Last AI report: healthy", "Replacement status: eligible review"].map((item) => <p key={item} className="mt-3 rounded-2xl bg-white/10 px-4 py-3 text-sm">{item}</p>)}</div></div></div>
      </div>
    </section>
  );
}
...
export function BeforeAfter() {
  return <section className="gl-reveal py-24"><div className="gl-container grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-center"><div><Badge tone="success">Before / After Slider</Badge><h2 className="mt-5 text-4xl font-semibold leading-tight sm:text-6xl">A transformation slider built for premium landscaping proof.</h2><p className="mt-5 text-base leading-8 text-neutral-slate">Hover the visual divider across unmanaged spaces and finished Garden Live landscapes.</p></div><div className="group relative h-[520px] overflow-hidden rounded-[2rem] border border-white bg-white shadow-[0_30px_100px_rgba(16,67,38,0.16)]"><Image src={img.before} alt="Before Garden Live landscaping" fill sizes="(min-width: 1024px) 60vw, 100vw" className="object-cover grayscale" /><div className="absolute inset-y-0 left-0 w-[58%] overflow-hidden transition-all duration-700 group-hover:w-[82%]"><Image src={img.after} alt="After Garden Live luxury landscaping" fill sizes="(min-width: 1024px) 60vw, 100vw" className="max-w-none object-cover" /></div><div className="absolute inset-y-8 left-[58%] w-1 rounded-full bg-white shadow-[0_0_40px_rgba(255,255,255,0.9)] transition-all duration-700 group-hover:left-[82%]" /><div className="absolute bottom-5 left-5 rounded-full bg-white/84 px-4 py-2 text-sm font-semibold backdrop-blur-xl">Before</div><div className="absolute bottom-5 right-5 rounded-full bg-botanical-green/90 px-4 py-2 text-sm font-semibold text-white backdrop-blur-xl">After Garden Live</div></div></div></section>;
}
