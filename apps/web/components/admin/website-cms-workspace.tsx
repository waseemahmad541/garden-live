...
type CmsModule = { key: CmsModuleKey; label: string; href: string; description: string; icon: LucideIcon };
type UploadPreview = { name: string; url: string; size: string };
...
const cmsModules: CmsModule[] = [
  { key: "dashboard", label: "Dashboard", href: "/admin/website-cms", description: "Website statistics, pages, blogs, projects, gallery, visitors, forms and quick actions.", icon: BarChart3 },
  { key: "branding", label: "Branding", href: "/admin/website-cms/branding", description: "Logo, white logo, footer logo, favicon, app icon, colors, tagline and copyright.", icon: Palette },
  { key: "homepage", label: "Homepage Editor", href: "/admin/website-cms/homepage", description: "Hero, buttons, features, statistics, memberships, projects, testimonials, FAQ, contact and live preview.", icon: Home },
  { key: "gallery", label: "Gallery Manager", href: "/admin/website-cms/gallery", description: "Upload images, create albums, manage before-after media, nursery images, indoor plants and videos.", icon: GalleryHorizontalEnd },
  { key: "projects", label: "Project Manager", href: "/admin/website-cms/projects", description: "Add, edit and delete projects with gallery, location, area, completion date, client and category.", icon: Building2 },
  { key: "blog", label: "Blog CMS", href: "/admin/website-cms/blog", description: "Add blog posts, categories, featured images, SEO, tags, publish and draft workflow.", icon: BookOpen },
  { key: "services", label: "Service Manager", href: "/admin/website-cms/services", description: "Add services with icons, images, pricing and descriptions.", icon: Brush },
  { key: "memberships", label: "Membership Manager", href: "/admin/website-cms/memberships", description: "Create plans, prices, features, popular badge and enable or disable status.", icon: ShieldCheck },
  { key: "store", label: "Store CMS", href: "/admin/website-cms/store", description: "Manage categories, products, images, price, stock, offers and featured products.", icon: ShoppingBag },
  { key: "team", label: "Team", href: "/admin/website-cms/team", description: "Add team members with designation, photo and experience.", icon: Users },
  { key: "testimonials", label: "Testimonials", href: "/admin/website-cms/testimonials", description: "Customer name, photo, rating, review and video review management.", icon: Star },
  { key: "contact", label: "Contact", href: "/admin/website-cms/contact", description: "Phone, email, WhatsApp, address, Google Map and social links.", icon: MapPin },
  { key: "forms", label: "Forms", href: "/admin/website-cms/forms", description: "Book Free Visit, Contact Form, Newsletter, CSV export and resolved status.", icon: Mail },
  { key: "seo", label: "SEO", href: "/admin/website-cms/seo", description: "Meta title, description, OG image, keywords, robots and sitemap.", icon: Globe2 },
  { key: "settings", label: "Settings", href: "/admin/website-cms/settings", description: "Maintenance mode, Google Analytics, Facebook Pixel, Tag Manager, SMTP, SMS and WhatsApp.", icon: Settings },
  { key: "media", label: "Media Library", href: "/admin/website-cms/media", description: "Upload, folders, search, delete and replace image workflow.", icon: FileImage }
];

const sectionNames = ["Hero", "Features", "Statistics", "Membership", "Projects", "Testimonials", "FAQ", "Contact"];
const sampleForms = [
...
export function WebsiteCmsWorkspace({ activeModule }: { activeModule: CmsModuleKey }) {
  const active = cmsModules.find((item) => item.key === activeModule) ?? cmsModules[0];
  ...
  return (
    <div className="grid gap-6 2xl:grid-cols-[320px_1fr]">
      <AdminPanel className="2xl:sticky 2xl:top-28 2xl:self-start">
        ...
        <nav className="grid gap-1" aria-label="Website CMS modules">
          {cmsModules.map((item) => {
            ...
            return (
              <Link key={item.key} href={item.href} className={cn("flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-medium transition", selected ? "bg-lime-300 text-[#06120C]" : "text-emerald-50/72 hover:bg-white/10 hover:text-white")}>
      ...
      <div className="space-y-6">
        <AdminPanel>
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-[1fr_auto] md:items-center">
            <label className="relative block">
              <Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-emerald-50/45" aria-hidden />
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${active.label}`} className="min-h-12 w-full rounded-2xl border border-white/10 bg-white/10 pl-11 pr-4 text-sm text-white outline-none placeholder:text-emerald-50/45 focus:border-lime-200" />
            </label>
        ...
        {active.key === "services" ? <CrudCollection title="Service Manager" fields={["Service Name", "Icon", "Image", "Pricing", "Description"]} rows={["Landscaping", "Garden Maintenance", "Plant Nursery", "Dedicated Gardener"]} /> : null}
        {active.key === "memberships" ? <CrudCollection title="Membership Manager" fields={["Plan Name", "Monthly Price", "Yearly Price", "Features", "Popular Badge", "Enable Status"]} rows={["Plant Care", "Smart Garden", "Home Garden", "Premium Garden", "Luxury Garden"]} /> : null}
        {active.key === "store" ? <CrudCollection title="Store CMS" fields={["Category", "Product", "Image", "Price", "Stock", "Offer", "Featured Product"]} rows={["Indoor Plants", "Palm Collection", "Pots and Planters", "Fertilizers"]} /> : null}
        ...
        {active.key === "settings" ? <CrudCollection title="Website Settings" fields={["Maintenance Mode", "Google Analytics", "Facebook Pixel", "Google Tag Manager", "SMTP", "SMS", "WhatsApp"]} rows={["Analytics", "Email", "SMS", "WhatsApp"]} /> : null}
...
export function isCmsModule(value: string): value is Exclude<CmsModuleKey, "dashboard"> {
  return cmsModules.some((item) => item.key === value && item.key !== "dashboard");
}

export function cmsModuleTitle(value: CmsModuleKey) {
  return cmsModules.find((item) => item.key === value)?.label ?? "Website CMS";
}
...
function FormsManager({ page, setPage }: { page: number; setPage: (page: number) => void }) { return <AdminPanel><div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><AdminSectionHeader eyebrow="Forms" title="Public form submissions" description="Book Free Visit, Contact Form and Newsletter workflows." /><Button variant="secondary"><Download className="mr-2 h-4 w-4" />Download CSV</Button></div><AdminTable columns={["Type", "Name", "Contact", "Source", "Status"]} rows={sampleForms} /><div className="mt-5 flex items-center justify-between"><Button variant="secondary" disabled={page === 1} onClick={() => setPage(Math.max(1, page - 1))}><ChevronLeft className="mr-2 h-4 w-4" />Previous</Button><span className="text-sm text-emerald-50/65">Page {page}</span><Button variant="secondary" onClick={() => setPage(page + 1)}>Next<ChevronRight className="ml-2 h-4 w-4" /></Button></div></AdminPanel>; }
